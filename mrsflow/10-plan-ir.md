...
**Implementation reality and the binding requirement.** The emitter is hand-written Rust (`mrsflow-core/src/plan/fold.rs`, the `Dialect` trait + `Dbisam`); the grammar is a Prolog DCG that lives **outside this repo**, at `../DibDog/dbisam-dcg-project/` (`grammar/dcg.pl`, parse entry `tools/parse-to-term.pl`). Because they are two separate artefacts, "the grammar is the predicate" is only true if it is *enforced*: **every SQL string the `Dbisam` dialect emits MUST parse under the DBISAM DCG.** This is wired as the test `emitted_dbisam_sql_parses_under_the_dcg` (feeds emitted SQL to `scryer-prolog … parse-to-term.pl`, skips if the grammar/scryer aren't present). Do not hand-author DBISAM syntax (TOP placement, identifier quoting, date literals, …) from assumption or MSDN — derive it from the DCG and let that test be the gate. The grammar is canonical; where it has disagreed with the live engine the grammar was corrected, not bypassed. (This requirement was implicit in the original plan and a later agent built the emitter without it, shipping `SELECT TOP n` / `#date#` / `"ident"` bugs that the DCG would have rejected immediately — hence stating it here in full.)
