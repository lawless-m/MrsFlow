...
### B3. DROP / ALTER on a table fails after mrsflow has SELECTed from it (server `0x2B05 ExecuteError`)
- **Symptom:** `Exportmaster.Query: DBISAM ExecuteStatement rejected — reqcode 0x2B05, identifier: "<TABLE>"`. The PrepareStatement phase succeeds and echoes the table name back, but ExecuteStatement returns the previously-undocumented `0x2B05` error (same body shape as `0x2B02 PrepareError` — 8 zero bytes + length-prefixed identifier). Pre-fix, the same code path silently parsed the error body as a success result, producing nonsense `RowsAffected` like `1953719636` (= `0x74726168` = ASCII "hart") while the operation never actually executed.
- **Diagnosis (2026-05-27):** the trigger is opening a cursor against the table — `SetToBegin` + any fetch via `ReadFirstRecordBlock` / `ReadNextRecordBlock`. A fresh CREATE → DROP (no SELECT between) works; a single SELECT in between is enough to poison the table for any subsequent DROP/ALTER from this client. DBSYS in script mode (no cursor opened, only Prepare → Execute → Reset) drops the same table cleanly in the same session — wire capture in `Derek/dbisam-capture-create-select-drop.pcapng`. Adding `BeginDML (0x0316)` before Prepare, the DDL-flavour ExecuteStatement (`+23 = 0x00`), and `CloseCursor (0x00A0)` + `ResetStatement (0x0334)` cleanup after the cursor drain — none of those individually or together unblock the DROP. The remaining unknown is what message DBSYS sends to release a fully-opened cursor that we don't.
- **Workaround:** do schema management (DROP / ALTER) via DBSYS, not mrsflow. mrsflow is read-heavy in practice (extract pipeline) and the four DML keywords UPDATE/INSERT/DELETE plus CREATE TABLE all work end-to-end.
- **What we have:** the client now detects the `0x2B__` error family broadly and surfaces a real Rust error with the offending identifier (via `parse_prepare_error`), so the failure is visible instead of silent. Test in `mrsflow-cli/src/exportmaster/client.rs::tests`. The `EM_DML_DEBUG=1` env var dumps per-step bytes if you want to keep digging.
