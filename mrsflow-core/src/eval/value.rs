...
impl LazyOdbcState {
    ...
    ///
    /// Generated by the Plan IR emitter ([`crate::plan::emit`]) under the
    /// generic-ODBC dialect, which reproduces the long-standing output exactly
    /// (`"col"` quoting, bare table name, `0`/`1` booleans, single-quoted text
    /// with `''` escape, ANSI date/timestamp literals, no LIMIT). The
    ...
    pub fn render_sql(&self) -> String {
        ...
        crate::plan::emit(&self.to_plan(), &crate::plan::GenericOdbc)
