...
/// so they show up in `#shared` and in `Expression.Evaluate` resolution.
/// Calling one raises NotImplemented. Sourced from Excel's actual
/// `Record.FieldNames(#shared)` (captured as Oracle/cases/q1165.excel.out)
/// minus the names mrsflow already exposes as real bindings.
///
/// Maintenance: regenerate from a fresh Excel capture when PQ adds names —
/// don't hand-edit individual entries.
const PQ_NAMES_STUB: &[&str] = &[
    ...
    "Occurrence.Required",
    "Occurrence.Type",
    "Office.InferChartPropertiesGenerator",
    "OleDb.DataSource",
    "OleDb.Query",
    "Oracle.Database",
    "Order.Type",
