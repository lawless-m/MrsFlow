...
fn contents(args: &[Value], host: &dyn IoHost) -> Result<Value, MError> {
    ...
    //
    // PQ rejects relative paths with "The supplied file path must be a
    // valid absolute path." but mrsflow accepts them — the coverage
    // dashboard (q1167) deliberately uses a relative path, and we want
    // local-script workflows to keep working. The byte-match cost is
    // q1167 staying on the DIFF list (error wording differs).
    let bytes = host
        .file_read(path)
